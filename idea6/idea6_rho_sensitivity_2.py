...
RHO_LIMIT_GRID = np.round(np.arange(0.45, 0.751, 0.01), 2).tolist()  # 0.01 간격, 31개
K_GRID_STEP = 5    # 25 -> 5 로 세밀화 (per_K 1회 계산이라 비용 증가는 미미)
...
def main():
    ...
    print('=' * 90)
    print('  rho^Limit x N_Memb 상세 (대표 지점: 0.05 간격)')
    print('=' * 90)
    print(f'  {"rho_Limit":>10} {"N_Memb":>7} {"K*":>6} {"N_eff":>6} '
          f'{"ECC":>8} {"rho_eff":>8}')

    REPRESENTATIVE_RHOS = [0.50, 0.55, 0.60, 0.65, 0.70]
    for rho_limit in REPRESENTATIVE_RHOS:
    ...
    # ---------- 전체 세밀 스윕(0.01 간격, 31개) : N_Memb 전부 훑어 최적(ECC 최댓값) 조합만 기록 ----------
    print('\n' + '=' * 90)
    print(f'  전체 세밀 스윕 요약 (rho_Limit 0.45~0.75, 0.01 간격, {len(RHO_LIMIT_GRID)}개 지점)')
    print('=' * 90)
    ...
    for rho_limit in RHO_LIMIT_GRID:
        ...
        n_memb, K_s, neff, ecc, rho = best_row
        shift = ''
        if prev_K is not None and K_s != prev_K:
            shift = f'  (K* {"증가" if K_s>prev_K else "감소"})'
    ...
    top_cluster_cache = {}   # K* 중복 계산 방지 (세밀 스윕에서 같은 K*가 자주 반복됨)
    prev_printed_K = None
    for rho_limit in RHO_LIMIT_GRID:
        if rho_limit not in summary:
            continue
        K_s = summary[rho_limit][1]
        if K_s == prev_printed_K:
            continue   # 바로 이전과 K*가 같으면 결과도 같으므로 중복 출력 생략
        if K_s not in top_cluster_cache:
            top_cluster_cache[K_s] = get_top_clusters_obs(Z, boxes, K_s, N_MEMB_FIXED, topN=3)
        top_clusters = top_cluster_cache[K_s]
        jvals = []
