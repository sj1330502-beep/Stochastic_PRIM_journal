...
# ============================================= [1] 합성 데이터 생성 (실제 상관구조 보존)
def load_real_X(csv_path):
    """실제 콘크리트 관측치 행렬을 그대로 로드 (상관구조·마진분포 참조용)"""
    df = pd.read_csv(csv_path, encoding='utf-8-sig')
    return df.iloc[:, :8].values.astype(float)


def sample_correlated_inputs(real_X, N, seed):
    """
    Gaussian copula 기반 관측치 생성.
    - 상관구조: 실제 데이터의 상관행렬을 그대로 사용 (독립 가정을 깨고
      Water-Superplast, Cement-FlyAsh 등 실제 변수 간 관계를 보존)
    - 마진 분포: 각 변수의 실제 경험적 분포(모양·왜도까지)를 그대로 따름
      (단순 균등분포가 아니라, 실제 값 분포의 quantile로 역매핑)
    이렇게 하면 관측치의 범위뿐 아니라 '변수들이 함께 움직이는 방식'과
    '각 변수의 실제 분포 모양'까지 실제 데이터와 닮게 된다.
    """
    rng = np.random.default_rng(seed)
    P = real_X.shape[1]

    # 1) 실제 데이터의 상관구조 추정 (Spearman: 순위상관 -> 코퓰라에 적합)
    ranks = np.apply_along_axis(lambda c: pd.Series(c).rank().values, 0, real_X)
    corr = np.corrcoef(ranks, rowvar=False)
    corr = (corr + corr.T) / 2
    np.fill_diagonal(corr, 1.0)

    # 2) 상관된 표준정규 샘플 생성 -> 균등분포로 변환 (Gaussian copula)
    Z = rng.multivariate_normal(mean=np.zeros(P), cov=corr, size=N)
    from scipy.stats import norm
    U = norm.cdf(Z)   # (N,P), 각 열은 개별적으로 균등(0,1), 열 간에는 실제 상관구조 보존

    # 3) 각 변수의 실제 경험적 분포로 역매핑 (quantile mapping)
    X_synth = np.zeros((N, P))
    for p in range(P):
        sorted_real = np.sort(real_X[:, p])
        ecdf_pos = np.linspace(0, 1, len(sorted_real))
        X_synth[:, p] = np.interp(U[:, p], ecdf_pos, sorted_real)
...
def sample_centers(real_X, G, min_sep_ratio, seed):
    ...
    while len(centers) < G and attempts < 5000:
        cand = real_X[rng.integers(0, N_real)]   # 실제 관측치 하나를 중심 후보로
...
def generate_synthetic_data(real_X, N, G, A, sigma_ratio, noise_std,
    ...
    X = sample_correlated_inputs(real_X, N, seed)   # 상관구조 보존된 관측치
...
def main():
    ...
    print(f'실제 관측치 {len(real_X)}개 로드 완료 (상관구조/마진분포 보존용)\n')
    ...
    for G in [2, 3, 5]:
        r = run_scenario(real_X, f'G={G}', N=1200, G=G,
