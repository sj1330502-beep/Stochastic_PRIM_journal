...
def score_against_truth(boxes, lab, n_memb_star, truth_sets, N_total):
    ...
    # 정답 영역 g마다 "가장 많이 겹치는 발견된 클러스터"와의 관측치 공유 비율
    # (1:1 그리디 매칭에 묶이지 않고, 매 정답 영역 기준으로 최선의 겹침을 그대로 보고)
    best_per_truth = []
    for gi, T in enumerate(truth_sets):
        best = (0.0, 0.0, 0.0, None)  # f1, prec, rec, ci
        for ci, C in enumerate(eff_clusters):
            inter = len(C & T)
            prec = inter / len(C) if C else 0.0
            rec = inter / len(T) if T else 0.0
            ...
            if f1 > best[0]:
                best = (f1, prec, rec, ci)
        best_per_truth.append(dict(truth_idx=gi, best_cluster=best[3],
...
def run_scenario(real_X, label, N=1000, G=3, A=None, sigma_ratio=0.15,
    ...
    result = dict(label=label, G=G, N_eff=score['N_eff'],
                  ...
                  best_per_truth=score['best_per_truth'],
    ...
    if verbose:
        ...
        # 정답 영역별로 "가장 잘 겹치는 발견된 클러스터"가 관측치를 얼마나 공유하는지
        for bpt in score['best_per_truth']:
            if bpt['best_cluster'] is None:
                print(f'      정답영역#{bpt["truth_idx"]}: 겹치는 클러스터 없음')
            else:
                print(f'      정답영역#{bpt["truth_idx"]} <-> 클러스터#{bpt["best_cluster"]}: '
                      f'공유비율(recall)={bpt["recall"]:.3f}, 정밀도(precision)={bpt["precision"]:.3f}, '
                      f'F1={bpt["f1"]:.3f}')
    return result


# ============================================= G값을 여러 개로 스왑하며 평균/표준편차 요약 (고정 시드)
def run_G_sweep(real_X, G_list, seed=0, **scenario_kwargs):
    """
    무작위 시드를 여러 번 도는 대신(시간이 오래 걸림), 시드는 고정하고
    G(정답 개수)만 여러 값으로 바꿔가며 실행 -> 그 결과들의 평균/표준편차로
    '원 논문 방법론이 G 값에 상관없이 얼마나 안정적으로 작동하는지'를 요약한다.
    """
    scenario_kwargs.pop('G', None)
    scenario_kwargs.pop('A', None)
    scenario_kwargs.pop('seed', None)
    scenario_kwargs.pop('verbose', None)

    runs = []
    for G in G_list:
        r = run_scenario(real_X, f'G={G}', G=G, A=[0.7] * G, seed=seed,
                         verbose=True, **scenario_kwargs)
        runs.append(r)

    valid = [r for r in runs if 'error' not in r]
    failed = len(G_list) - len(valid)
    if not valid:
        print(f'  [G-sweep] 전체 {len(G_list)}개 G값 모두 실패 (박스/클러스터 생성 불가)')
        return dict(label='G-sweep', G_list=G_list, n_failed=failed, runs=runs)
    ...
    summary = dict(
        label='G-sweep', G_list=G_list, n_failed=failed,
        f1_mean=f1s.mean(), f1_std=f1s.std(),
        count_match_rate=count_matches.mean(),
        n_eff_mean=n_effs.mean(), n_eff_std=n_effs.std(),
        seed=seed, runs=runs,
    )
    print(f'  [G-sweep 요약] G={G_list} (seed={seed}) | '
          f'{len(valid)}/{len(G_list)}개 성공 (실패 {failed}개) | '
...
def main(G_list=(2, 3, 4, 5, 6, 7, 8)):
    ...
    print('\n' + '=' * 78)
    print('  난이도별 정답 복원 검증 (recipe-replicate 구조 사용, 시드 고정)')
    print('=' * 78)

    summaries = []

    print(f'\n[A] 정답 개수(G) 스왑 (거리/noise 고정, seed=1) — 평균/표준편차 산출')
    s = run_G_sweep(real_X, G_list=list(G_list), seed=1, N=1200,
                    sigma_ratio=0.15, noise_std=0.05, min_sep_ratio=0.20)
    summaries.append(s)

    print('\n[B] 중심 간 거리(min_sep_ratio) 변화 (G=3 고정)')
    for sep in [0.10, 0.20, 0.35]:
        r = run_scenario(real_X, f'sep={sep}', N=1200, G=3,
    ...
    for noise in [0.02, 0.05, 0.10]:
        r = run_scenario(real_X, f'noise={noise}', N=1200, G=3,
    ...
    for A in ([0.7, 0.7, 0.7], [0.9, 0.5, 0.2]):
        r = run_scenario(real_X, f'A={A}', N=1200, G=3,
