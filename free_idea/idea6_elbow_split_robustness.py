...
TRAIN_RATIO = 0.7
SPLIT_SEEDS = [int(x) for x in np.random.default_rng().integers(0, 1_000_000, size=10)]
...
def run_one_seed(df, split_seed):
    ...
    return dict(split_seed=split_seed, K_o=K_o, n_memb_o=n_memb_o, d_o=d_o, n_o=n_o,
               K_e=K_e, n_memb_e=n_memb_e, d_e=d_e, n_e=n_e)
...
def main():
    ...
    for seed in SPLIT_SEEDS:
        ...
        results.append(r)
        diff = r['d_e'] - r['d_o']
        print(f'  기존: N_Memb*={r["n_memb_o"]}, K*={r["K_o"]}, D_conf={r["d_o"]:.4f} (n={r["n_o"]})')
        print(f'  엘보우: N_Memb*={r["n_memb_e"]}, K*={r["K_e"]}, D_conf={r["d_e"]:.4f} '
              f'(n={r["n_e"]}), 차이={diff:+.4f}')

    print('\n' + '=' * 90)
    print('  종합 (분할 시드별 요약)')
    print('=' * 90)
    print(f'  {"시드":>6} {"기존 D_conf":>12} {"엘보우 D_conf":>14} {"차이":>9}')
    diffs = []
    for r in results:
        diff = r['d_e'] - r['d_o']
        diffs.append(diff)
        print(f'  {r["split_seed"]:>6} {r["d_o"]:>12.4f} {r["d_e"]:>14.4f} {diff:>+9.4f}')

    if diffs:
        wins = sum(1 for d in diffs if d >= 0)
        strict_wins = sum(1 for d in diffs if d > 0)
        ties = sum(1 for d in diffs if d == 0)
        print(f'\n  엘보우가 기존과 같거나 능가한 분할 수: {wins}/{len(diffs)} '
