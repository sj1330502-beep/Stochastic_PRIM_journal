...
TRAIN_RATIO = 0.7
N_RANDOM_SEEDS = 10   # 매 실행마다 새로 뽑는 랜덤 시드 개수
...
def build_consensus_votes(raw, boxes, X_train, k_grid, n_memb_options):
    n_train = len(X_train)
    votes = np.zeros(n_train, dtype=int)
    n_settings = 0
    for n_memb in n_memb_options:
        for K in k_grid:
            eff = effective_clusters(raw[K], n_memb)
            ...
            all_idx = np.concatenate([boxes[i]['idx'] for i in largest['mem']])
            unique_idx = np.unique(all_idx)
            votes[unique_idx] += 1
            n_settings += 1
    return votes, n_settings


def consensus_range(votes, n_settings, X_train, threshold_ratio):
    min_votes = threshold_ratio * n_settings
    chosen = np.where(votes >= min_votes)[0]
    if len(chosen) < 5:
        return None, None, 0
    lo = X_train[chosen].min(axis=0)
    hi = X_train[chosen].max(axis=0)
    return lo, hi, len(chosen)
...
def run_one_seed(df, split_seed):
    ...
    votes, n_settings = build_consensus_votes(raw, boxes, X_train, k_grid, N_MEMB_OPTIONS)
    best_consensus = None
    for thr in VOTE_THRESHOLDS:
        lo, hi, n_chosen = consensus_range(votes, n_settings, X_train, thr)
        if lo is None:
            continue
        d_c, n_c = eval_range_on_confirm(lo, hi, X_confirm, D_confirm)
        if best_consensus is None or d_c > best_consensus[1]:
            best_consensus = (thr, d_c, n_c)
...
def main():
    ...
    print('=' * 90)
    print(f'  서로 다른 {len(SPLIT_SEEDS)}개 training/confirmation 분할로 반복 검증 (랜덤)')
    print(f'  (원논문 vs 합의영역Consensus Region)')
    print('=' * 90)
    ...
    for seed in SPLIT_SEEDS:
        ...
        if r is None:
            ...
            continue
        results.append(r)
        thr, d_c, n_c = r['best_consensus']
        diff = d_c - r['d_o']
        print(f'  기존: K*={r["K_o"]}, D_conf={r["d_o"]:.4f} (n={r["n_o"]})')
        print(f'  합의영역(최고 득표율={thr}): D_conf={d_c:.4f} (n={n_c}), 차이={diff:+.4f}')

    print('\n' + '=' * 90)
    print('  종합 (분할 시드별 요약)')
    print('=' * 90)
    print(f'  {"시드":>6} {"기존 D_conf":>12} {"합의영역 D_conf":>16} {"차이":>9}')
    diffs = []
    for r in results:
        thr, d_c, n_c = r['best_consensus']
        diff = d_c - r['d_o']
        diffs.append(diff)
        print(f'  {r["split_seed"]:>6} {r["d_o"]:>12.4f} {d_c:>16.4f} {diff:>+9.4f}')

    if diffs:
        wins = sum(1 for d in diffs if d >= 0)
        print(f'\n  합의영역이 기존과 같거나 능가한 분할 수: {wins}/{len(diffs)}')
